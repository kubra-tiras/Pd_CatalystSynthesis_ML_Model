...
k=5 # 5-fold cross validation (reference paper)
...
for (j in seq(from=50, to=200, by=50))
{
  for (n in seq(from=5, to=50, by=5))
  {
...
resultfile <- write_xlsx(results)
  ...
  NTREE_OPT <- 50   # PUT THE OPTIMUM TREE NUMBER BY USING RMSE, ERROR, R-SQUARE FROM ABOVE
...
# Train Data Results:
  ...
  prediction_corrected = prediction_train*1.22-5.95
...
# Test Model with Test Data:  
  ...
  prediction_test_corrected=prediction_test*1.22-5.95
  ...
  prediction_exp_cf = prediction_exp*1.22-5.95
  prediction_exp_cf
